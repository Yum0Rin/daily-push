...
```
daily-push/
├── start.py                     # 本地入口：默认「采集+服务」；--serve-only 只服务（按需网页）
...
│   ├── pipeline.py              # 采集流水线：网易云代理生命周期 + 一次 采集→导出→推送
...
```
每天（登录时本地一次性采集 / 07:30 云端 GitHub Actions）
        │
        ▼
...
本地查看：Flask 仪表盘  http://127.0.0.1:5000（**按需启停**：从开始菜单「每日推送」打开；
设置页「停止本地服务」关闭；或直接看 Pages 静态链接）
```
...
python start.py --serve-only       # 按需起本地网页（不采集、不定时）
...
- ✅ 本地设置页 `/settings`：可视化编辑屏蔽名单 / 平台登录态（含 Cookie 更新与验证）/ 采集参数；「手动推送」一键跑定时流程（采集→导出→推送 Pages）；「停止本地服务」按需关闭网页+代理；仅本机可访问。
