...
| `netease.max_favorites` | 日推收藏数阈值 | 收藏数**大于**该值的歌跳过、顺延下一首；`0`=不限（默认 0）。存于 `settings.json` |
| `netease.reserve` / `bilibili.reserve` / `wechat.reserve` | 各源隐藏缓冲条数 | 默认 3；多取的条目标 `hidden` 供回填，前端不显示。存于 `settings.json` |
